...
if __name__ == "__main__":
    ...
    if args.model_name == 'EdgeBank':
        ...
        for run in range(args.num_runs):
            ...
            one=args.onehot
            args.seed = run
            args.load_model_name = f'{args.model_name}_seed{args.seed}_{name}'
            args.save_result_name = f'{args.negative_sample_strategy}_negative_sampling_{args.model_name}_seed{args.seed}_{name}'
            ...
            elif args.model_name == 'ChordMixer':
                dynamic_backbone = ChordMixer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=full_neighbor_sampler,
                                              time_feat_dim=args.time_feat_dim, num_tokens=args.num_neighbors, num_layers=args.num_layers, dropout=args.dropout, device=args.device)
            ...
            elif args.model_name == 'DyGMixer':
                dynamic_backbone = DyGFMixer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=full_neighbor_sampler,
                                             time_feat_dim=args.time_feat_dim, channel_embedding_dim=args.channel_embedding_dim, patch_size=args.patch_size,
                                             num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout,
                                             max_input_sequence_length=args.max_input_sequence_length, device=args.device)
            else:
