...
from models.DyGMixer import DyGMixer
from models.modules import MergeLayer,DownstreamLayer,HiddenLayer,ProjectionLayer,NewLayer,onehotLayer
from utils.utils import set_random_seed, convert_to_gpu, get_parameter_sizes, create_optimizer,loss_cl
...
if __name__ == "__main__":
    ...
    node_raw_features, edge_raw_features, full_data, train_data, val_data, test_data, new_node_val_data, new_node_test_data = \
        get_link_prediction_data(dataset_name=args.dataset_name, val_ratio=args.val_ratio, test_ratio=args.test_ratio, sample_ratio=args.sample_ratio)
    ...
    for run in range(args.num_runs):
        ...
        hidden_layer=HiddenLayer(input_dim1=node_raw_features.shape[1], input_dim2=node_raw_features.shape[1],
                                 hidden_dim=node_raw_features.shape[1])
        if args.onehot=='origin':
            link_predictor = NewLayer(hidden_dim=node_raw_features.shape[1])
        else:
            link_predictor = onehotLayer(onehot_dim=node_raw_features.shape[0],hidden_dim=172,filter=args.filter)
        proj_head = ProjectionLayer(hidden_dim=node_raw_features.shape[1])
        # link_predictor = DownstreamLayer(input_dim1=node_raw_features.shape[1], input_dim2=node_raw_features.shape[1],
        #                             hidden_dim=node_raw_features.shape[1], output_dim=1)
        # model = nn.Sequential(dynamic_backbone,hidden_layer, proj_head, link_predictor)
        ...
        for epoch in range(args.num_epochs):
            ...
            model.train()
            if args.model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'ChordMixer','DyGFormer','DyGMixer']:
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    batch_src_node_embeddings, batch_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                          ...
                                                                          num_neighbors=args.num_neighbors)
                elif args.model_name in ['GraphMixer','ChordMixer']:
                    # get temporal embedding of source and destination nodes
                    # two Tensors, with shape (batch_size, node_feat_dim)
                    batch_src_node_embeddings, batch_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                    ...
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                          ...
                                                                          num_neighbors=args.num_neighbors,
                                                                          time_gap=args.time_gap)
                elif args.model_name in ['DyGFormer','DyGMixer']:
                    # get temporal embedding of source and destination nodes
                    # two Tensors, with shape (batch_size, node_feat_dim)
                    batch_src_node_embeddings, batch_dst_node_embeddings = \
                        model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                ...
                loss = loss_func(positive_probabilities, torch.ones_like(positive_probabilities)) + loss_func(negative_probabilities, torch.zeros_like(negative_probabilities))


                predicts = torch.cat([positive_probabilities, negative_probabilities], dim=0).view(-1)
                labels = torch.cat([torch.ones(positive_probabilities.size(0)), torch.zeros(negative_probabilities.size(0))], dim=0).view(-1)
    ...
    args.save_avg_name = f'{args.model_name}_avg_{name}'
    save_avg_result_path = os.path.join(save_result_folder, f"{args.save_avg_name}.json")
