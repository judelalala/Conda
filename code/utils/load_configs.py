...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model, note that EdgeBank is only applicable for evaluation',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'EdgeBank', 'TCL', 'GraphMixer', 'ChordMixer','DyGFormer','DyGMixer'])
    ...
    parser.add_argument('--aug', type=str, default='origin',
                        choices=['origin', 'dsu', 'UmmU','allUmmU'],
                        help='augmentation method')
    parser.add_argument('--sample_ratio', type=float, default=1.0, help='augmentation method')

    parser.add_argument('--onehot', type=str, default='origin', choices=['raw','onehot'],help='whether use one-hot')
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        if args.dataset_name in ['reddit', 'CanParl', 'USLegis', 'UNtrade', 'UNvote']:
            ...
            args.sample_neighbor_strategy = 'recent'
    elif args.model_name in ['ChordMixer','GraphMixer']:
        args.num_layers = 2
...
def get_node_classification_args():
    ...
    parser.add_argument('--test_interval_epochs', type=int, default=10, help='how many epochs to perform testing once')
    parser.add_argument('--load_best_configs', action='store_true', default=False, help='whether to load the best configurations')
    parser.add_argument('--filter', type=str, default='False', choices=['False','adap', 'low','band','high','test','test1'],help='whether use filter')
    try:
        args = parser.parse_args()
        args.device = f'cuda:{args.gpu}' if torch.cuda.is_available() and args.gpu >= 0 else 'cpu'
    except:
