...
# Ensures the random sampler uses a fixed seed for evaluation (i.e. we always sample the same negatives for validation / test set)
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'ChordMixer', 'DyGFormer','DyGMixer']:
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                      ...
                                                                      num_neighbors=num_neighbors)
            elif model_name in ['GraphMixer','ChordMixer']:
                # get temporal embedding of source and destination nodes
                # two Tensors, with shape (batch_size, node_feat_dim)
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                ...
                batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                      ...
                                                                      num_neighbors=num_neighbors,
                                                                      time_gap=time_gap)
            elif model_name in ['DyGFormer','DyGMixer']:
                # get temporal embedding of source and destination nodes
                # two Tensors, with shape (batch_size, node_feat_dim)
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
            ...
            # positive_probabilities = model[-1](positive_emb, stat='pos').squeeze(dim=-1).sigmoid()
            # negative_probabilities = model[-1](negative_emb, stat='neg').squeeze(dim=-1).sigmoid()



            if args.onehot=='onehot':
                one_hot_embeddings=torch.eye(node_raw_features.shape[0]).to(args.device)
                batch_src_node_embeddings=torch.cat([batch_src_node_embeddings,one_hot_embeddings[batch_src_node_ids]],dim=-1)
                batch_dst_node_embeddings=torch.cat([batch_dst_node_embeddings,one_hot_embeddings[batch_dst_node_ids]],dim=-1)
                batch_neg_src_node_embeddings=torch.cat([batch_neg_src_node_embeddings,one_hot_embeddings[batch_neg_src_node_ids]],dim=-1)
                batch_neg_dst_node_embeddings=torch.cat([batch_neg_dst_node_embeddings,one_hot_embeddings[batch_neg_dst_node_ids]],dim=-1)
            ...
            loss = loss_func(positive_probabilities, torch.ones_like(positive_probabilities)) + loss_func(
            ...
            labels = torch.cat(
                [torch.ones(positive_probabilities.size(0)), torch.zeros(negative_probabilities.size(0))], dim=0).view(
                -1)
